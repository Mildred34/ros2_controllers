...
#include "actuator_trajectory_controller/actuator_trajectory_controller.hpp"
...
#include "actuator_trajectory_controller/trajectory.hpp"
...
{
ActuatorTrajectoryController::ActuatorTrajectoryController()
: controller_interface::ControllerInterface(), dof_(0)
{
}

controller_interface::CallbackReturn ActuatorTrajectoryController::on_init()
{
  try
  {
...
ActuatorTrajectoryController::command_interface_configuration() const
{
  ...
  for (const auto & actuator_name : command_actuator_names_)
  {
    for (const auto & interface_type : params_.command_interfaces)
    {
      conf.names.push_back(actuator_name + "/" + interface_type);
    }
  }
  return conf;
}
...
controller_interface::InterfaceConfiguration
ActuatorTrajectoryController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration conf;
  conf.type = controller_interface::interface_configuration_type::INDIVIDUAL;
  conf.names.reserve(dof_ * params_.state_interfaces.size());
  for (const auto & actuator_name : params_.actuators)
  {
...
controller_interface::return_type ActuatorTrajectoryController::update(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  auto compute_error_for_actuator = [&](
                                   ActuatorTrajectoryPoint & error, int index,
                                   const ActuatorTrajectoryPoint & current,
                                   const ActuatorTrajectoryPoint & desired)
  {
  ...
  if (current_external_msg != *new_external_msg)
  {
    ...
    sort_to_local_actuator_order(*new_external_msg);
    // TODO(denis): Add here integration of position and velocity
    traj_external_point_ptr_->update(*new_external_msg);
  }

  // TODO(anyone): can I here also use const on actuator_interface since the reference_wrapper is not
  // changed, but its value only?
  auto assign_interface_from_point =
    [&](auto & actuator_interface, const std::vector<double> & trajectory_point_interface)
  {
    for (size_t index = 0; index < dof_; ++index)
    {
      actuator_interface[index].get().set_value(trajectory_point_interface[index]);
    }
  };
  ...
  if (traj_point_active_ptr_ && (*traj_point_active_ptr_)->has_trajectory_msg())
  {
    ...
    if (valid_point)
    {
      ...
      for (size_t index = 0; index < dof_; ++index)
      {
        compute_error_for_actuator(state_error_, index, state_current_, state_desired_);

        // Always check the state tolerance on the first sample in case the first sample
        // is the last point
        if (
          (before_last_point || first_sample) &&
          !check_state_tolerance_per_actuator(
      ...
      if (!tolerance_violated_while_moving && within_goal_time)
      {
        ...
        {
          assign_interface_from_point(actuator_command_interface_[0], state_desired_.positions);
        }
        if (has_velocity_command_interface_)
        {
          if (use_closed_loop_pid_adapter_)
          {
            assign_interface_from_point(actuator_command_interface_[1], tmp_command_);
          }
          else
          {
            assign_interface_from_point(actuator_command_interface_[1], state_desired_.velocities);
          }
        }
        if (has_acceleration_command_interface_)
        {
          assign_interface_from_point(actuator_command_interface_[2], state_desired_.accelerations);
        }
        if (has_effort_command_interface_)
        {
          if (use_closed_loop_pid_adapter_)
          {
            assign_interface_from_point(actuator_command_interface_[3], tmp_command_);
          }
          else
          {
            assign_interface_from_point(actuator_command_interface_[3], state_desired_.effort);
          }
        }
      ...
      if (active_goal)
      {
        ...
        feedback->joint_names = params_.actuators; // FollowJTrajAction::Feedback
...
void ActuatorTrajectoryController::read_state_from_hardware(ActuatorTrajectoryPoint & state)
{
  auto assign_point_from_interface =
    [&](std::vector<double> & trajectory_point_interface, const auto & actuator_interface)
  {
    for (size_t index = 0; index < dof_; ++index)
    {
      trajectory_point_interface[index] = actuator_interface[index].get().get_value();
    }
  };

  // Assign values from the hardware
  // Position states always exist
  assign_point_from_interface(state.positions, actuator_state_interface_[0]);
  // velocity and acceleration states are optional
  if (has_velocity_state_interface_)
  {
    assign_point_from_interface(state.velocities, actuator_state_interface_[1]);
    // Acceleration is used only in combination with velocity
    if (has_acceleration_state_interface_)
    {
      assign_point_from_interface(state.accelerations, actuator_state_interface_[2]);
    }
    else
    {
...
bool ActuatorTrajectoryController::read_state_from_command_interfaces(ActuatorTrajectoryPoint & state)
{
  ...
  auto interface_has_values = [](const auto & actuator_interface)
  {
    return std::find_if(
             actuator_interface.begin(), actuator_interface.end(),
             [](const auto & interface)
             { return std::isnan(interface.get().get_value()); }) == actuator_interface.end();
  };
  ...
  if (has_position_command_interface_ && interface_has_values(actuator_command_interface_[0]))
  {
    assign_point_from_interface(state.positions, actuator_command_interface_[0]);
  }
  else
  {
  ...
  if (has_velocity_state_interface_)
  {
    if (has_velocity_command_interface_ && interface_has_values(actuator_command_interface_[1]))
    {
      assign_point_from_interface(state.velocities, actuator_command_interface_[1]);
    }
    else
    {
      state.velocities.clear();
  ...
  if (has_acceleration_state_interface_)
  {
    if (has_acceleration_command_interface_ && interface_has_values(actuator_command_interface_[2]))
    {
      assign_point_from_interface(state.accelerations, actuator_command_interface_[2]);
    }
    else
    {
      state.accelerations.clear();
...
controller_interface::CallbackReturn ActuatorTrajectoryController::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  if ((dof_ > 0) && (params_.actuators.size() != dof_))
  {
    RCLCPP_ERROR(
      logger,
      "The ActuatorTrajectoryController does not support restarting with a different number of DOF");
    // TODO(andyz): update vector lengths if num. actuators did change and re-initialize them so we
    // can continue
    return CallbackReturn::FAILURE;
  }

  dof_ = params_.actuators.size();
  ...
  if (params_.actuators.empty())
  {
    // TODO(destogl): is this correct? Can we really move-on if no actuators names are not provided?
    RCLCPP_WARN(logger, "'actuators' parameter is empty.");
  }

  command_actuator_names_ = params_.command_actuators;

  if (command_actuator_names_.empty())
  {
    command_actuator_names_ = params_.actuators;
    RCLCPP_INFO(
      logger, "No specific actuator names are used for command interfaces. Using 'actuators' parameter.");
  }
  else if (command_actuator_names_.size() != params_.actuators.size())
  {
    RCLCPP_ERROR(
      logger, "'command_actuators' parameter has to have the same size as 'actuators' parameter.");
    return CallbackReturn::FAILURE;
  }
  ...
  actuator_command_interface_.resize(allowed_interface_types_.size());
  ...
  if (use_closed_loop_pid_adapter_)
  {
    ...
    {
      const auto & gains = params_.gains.actuators_map.at(params_.actuators[i]);
      ...
      if (gains.ff_velocity_scale == 0.0)
      {
        RCLCPP_WARN(
          get_node()->get_logger(),
          "'ff_velocity_scale' parameters is not defined under 'gains.<actuator_name>.' structure. "
          "Maybe you are using deprecated format 'ff_velocity_scale/<actuator_name>'!");

        ff_velocity_scale_[i] = auto_declare<double>("ff_velocity_scale/" + params_.actuators[i], 0.0);
      }
      else
      {
  ...
  actuator_state_interface_.resize(allowed_interface_types_.size());
  ...
  actuator_command_subscriber_ =
    get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
      "~/actuator_trajectory", rclcpp::SystemDefaultsQoS(),
      std::bind(&ActuatorTrajectoryController::topic_callback, this, std::placeholders::_1));
  ...
  // We'll create update msg / action later on
  state_publisher_->msg_.joint_names = params_.actuators;
  ...
  if (params_.allow_partial_actuators_goal)
  {
    RCLCPP_INFO(logger, "Goals with partial set of actuators are allowed");
  }
  ...
  action_server_ = rclcpp_action::create_server<FollowJTrajAction>(
    ...
    std::string(get_node()->get_name()) + "/follow_actuator_trajectory",
    std::bind(&ActuatorTrajectoryController::goal_received_callback, this, _1, _2),
    std::bind(&ActuatorTrajectoryController::goal_cancelled_callback, this, _1),
    std::bind(&ActuatorTrajectoryController::goal_accepted_callback, this, _1));

  resize_actuator_trajectory_point(state_current_, dof_);
  resize_actuator_trajectory_point(state_desired_, dof_);
  resize_actuator_trajectory_point(state_error_, dof_);
  resize_actuator_trajectory_point(last_commanded_state_, dof_);

  return CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn ActuatorTrajectoryController::on_activate(
  const rclcpp_lifecycle::State &)
{
  // order all actuators in the storage
  for (const auto & interface : params_.command_interfaces)
  {
    ...
    if (!controller_interface::get_ordered_interfaces(
          command_interfaces_, command_actuator_names_, interface, actuator_command_interface_[index]))
    {
      RCLCPP_ERROR(
        get_node()->get_logger(), "Expected %zu '%s' command interfaces, got %zu.", dof_,
        interface.c_str(), actuator_command_interface_[index].size());
      return CallbackReturn::ERROR;
    }
  }
  for (const auto & interface : params_.state_interfaces)
  {
    ...
    auto index = std::distance(allowed_interface_types_.begin(), it);
    if (!controller_interface::get_ordered_interfaces(
          state_interfaces_, params_.actuators, interface, actuator_state_interface_[index]))
    {
      RCLCPP_ERROR(
        get_node()->get_logger(), "Expected %zu '%s' state interfaces, got %zu.", dof_,
        interface.c_str(), actuator_state_interface_[index].size());
      return CallbackReturn::ERROR;
    }
  }
  ...
  traj_msg_home_ptr_->points[0].positions.resize(actuator_state_interface_[0].size());
  for (size_t index = 0; index < actuator_state_interface_[0].size(); ++index)
  {
    traj_msg_home_ptr_->points[0].positions[index] =
      actuator_state_interface_[0][index].get().get_value();
  }
  ...
  resize_actuator_trajectory_point(state, dof_);
...
controller_interface::CallbackReturn ActuatorTrajectoryController::on_deactivate(
  const rclcpp_lifecycle::State &)
{
  ...
  for (size_t index = 0; index < dof_; ++index)
  {
    if (has_position_command_interface_)
    {
      actuator_command_interface_[0][index].get().set_value(
        actuator_command_interface_[0][index].get().get_value());
    }

    if (has_velocity_command_interface_)
    {
      actuator_command_interface_[1][index].get().set_value(0.0);
    }

    if (has_effort_command_interface_)
    {
      actuator_command_interface_[3][index].get().set_value(0.0);
    }
  }

  for (size_t index = 0; index < allowed_interface_types_.size(); ++index)
  {
    actuator_command_interface_[index].clear();
    actuator_command_interface_[index].clear();
  }
  release_interfaces();
...
controller_interface::CallbackReturn ActuatorTrajectoryController::on_cleanup(
  const rclcpp_lifecycle::State &)
{
  // go home
...
controller_interface::CallbackReturn ActuatorTrajectoryController::on_error(
  const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
...
bool ActuatorTrajectoryController::reset()
{
  subscriber_is_active_ = false;
  actuator_command_subscriber_.reset();
...
controller_interface::CallbackReturn ActuatorTrajectoryController::on_shutdown(
  const rclcpp_lifecycle::State &)
{
...
void ActuatorTrajectoryController::publish_state(
  const ActuatorTrajectoryPoint & desired_state, const ActuatorTrajectoryPoint & current_state,
  const ActuatorTrajectoryPoint & state_error)
{
...
void ActuatorTrajectoryController::topic_callback(
...
rclcpp_action::GoalResponse ActuatorTrajectoryController::goal_received_callback(
...
rclcpp_action::CancelResponse ActuatorTrajectoryController::goal_cancelled_callback(
...
void ActuatorTrajectoryController::goal_accepted_callback(
  std::shared_ptr<rclcpp_action::ServerGoalHandle<FollowJTrajAction>> goal_handle)
{
  ...
  rt_goal->preallocated_feedback_->joint_names = params_.actuators; // followtrajaction
  rt_goal->execute();
...
void ActuatorTrajectoryController::fill_partial_goal(
  std::shared_ptr<trajectory_msgs::msg::JointTrajectory> trajectory_msg) const
{
  // actuator names in the goal are a subset of existing actuators, as checked in goal_callback
  // so if the size matches, the goal contains all controller actuators
  ...
  {
    {
      if (
        std::find(
          ...
          params_.actuators[index]) != trajectory_msg->joint_names.end())
      {
        // actuator found on msg
        continue;
      }
      trajectory_msg->joint_names.push_back(params_.actuators[index]);

      for (auto & it : trajectory_msg->points)
      {
        // Assume hold position with 0 velocity and acceleration for missing actuators
        if (!it.positions.empty())
        {
          if (
            has_position_command_interface_ &&
            !std::isnan(actuator_command_interface_[0][index].get().get_value()))
          {
            // copy last command if cmd interface exists
            it.positions.push_back(actuator_command_interface_[0][index].get().get_value());
          }
          else if (has_position_state_interface_)
          {
            // copy current state if state interface exists
            it.positions.push_back(actuator_state_interface_[0][index].get().get_value());
          }
        }
...
void ActuatorTrajectoryController::sort_to_local_actuator_order(
  std::shared_ptr<trajectory_msgs::msg::JointTrajectory> trajectory_msg)
{
  // rearrange all points in the trajectory message based on mapping
  std::vector<size_t> mapping_vector = mapping(trajectory_msg->joint_names, params_.actuators);
  auto remap = [this](
...
bool ActuatorTrajectoryController::validate_trajectory_point_field(
  size_t actuator_names_size, const std::vector<double> & vector_field,
  const std::string & string_for_vector_field, size_t i, bool allow_empty) const
{
  ...
  if (actuator_names_size != vector_field.size())
  {
    RCLCPP_ERROR(
      get_node()->get_logger(), "Mismatch between actuator_names (%zu) and %s (%zu) at point #%zu.",
      actuator_names_size, string_for_vector_field.c_str(), vector_field.size(), i);
    return false;
  }
  return true;
}

bool ActuatorTrajectoryController::validate_trajectory_msg(
  const trajectory_msgs::msg::JointTrajectory & trajectory) const
{
  // If partial actuators goals are not allowed, goal should specify all controller actuators
  if (!params_.allow_partial_actuators_goal)
  {
    if (trajectory.joint_names.size() != dof_)
    {
      RCLCPP_ERROR(
        get_node()->get_logger(),
        "Actuators on incoming trajectory don't match the controller actuators.");
      return false;
    }
  }
  ...
  {
    RCLCPP_ERROR(get_node()->get_logger(), "Empty actuator names on incoming trajectory.");
    return false;
  }
  ...
  {
    const std::string & incoming_actuator_name = trajectory.joint_names[i];

    auto it = std::find(params_.actuators.begin(), params_.actuators.end(), incoming_actuator_name);
    if (it == params_.actuators.end())
    {
      RCLCPP_ERROR(
        get_node()->get_logger(), "Incoming actuator %s doesn't match the controller's actuators.",
        incoming_actuator_name.c_str());
      return false;
    }
  }
  ...
  for (size_t i = 0; i < trajectory.points.size(); ++i)
  {
    ...
    const size_t actuator_count = trajectory.joint_names.size();
    ...
    if (params_.allow_integration_in_goal_trajectories)
    {
      ...
      const bool position_error =
        ...
        !validate_trajectory_point_field(actuator_count, points[i].positions, "positions", i, false);
      const bool velocity_error =
        !points[i].velocities.empty() &&
        !validate_trajectory_point_field(actuator_count, points[i].velocities, "velocities", i, false);
      const bool acceleration_error =
        !points[i].accelerations.empty() &&
        !validate_trajectory_point_field(
          actuator_count, points[i].accelerations, "accelerations", i, false);
      ...
      !validate_trajectory_point_field(actuator_count, points[i].positions, "positions", i, false) ||
      !validate_trajectory_point_field(actuator_count, points[i].velocities, "velocities", i, true) ||
      !validate_trajectory_point_field(
        actuator_count, points[i].accelerations, "accelerations", i, true) ||
      !validate_trajectory_point_field(actuator_count, points[i].effort, "effort", i, true))
    {
      return false;
    }
  }
...
void ActuatorTrajectoryController::add_new_trajectory_msg(
  const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> & traj_msg)
{
  traj_msg_external_point_ptr_.writeFromNonRT(traj_msg);
}

void ActuatorTrajectoryController::preempt_active_goal()
{
  const auto active_goal = *rt_active_goal_.readFromNonRT();
  if (active_goal)
  {
...
void ActuatorTrajectoryController::set_hold_position()
{
...
bool ActuatorTrajectoryController::contains_interface_type(
...
void ActuatorTrajectoryController::resize_actuator_trajectory_point(
...
PLUGINLIB_EXPORT_CLASS(
  actuator_trajectory_controller::ActuatorTrajectoryController, controller_interface::ControllerInterface)
